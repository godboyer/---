<template>
  <section class="pr-6 pt-12 left-box">
    <n-list class="pt-12">
      ...
      <n-list-item v-for="item in house.HouseCardList" @click.stop.prevent="GoToHouseDetail(item.house_id)">
        <template #prefix>
        ...
        <n-thing :title="item?.title" :description="item?.location">
          ...
          <p>
            ...
            <span>
              <n-icon><svg width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <p class="tag">
            <n-tag round v-for="(v, idx) in item?.tags" ize="small" :color="{
              color: '#42B2B6',
              textColor: '#fff',
              borderColor: '#42B2B6',
            }" :key="idx">{{ v }}</n-tag>
